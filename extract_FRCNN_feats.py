...
from maskrcnn_benchmark.maskrcnn_benchmark.layers import nms
from maskrcnn_benchmark.maskrcnn_benchmark.modeling.detector import build_detection_model
from maskrcnn_benchmark.maskrcnn_benchmark.structures.image_list import to_image_list
from maskrcnn_benchmark.maskrcnn_benchmark.utils.model_serialization import load_state_dict
...
class FeatureExtractor:
    ...
    def get_parser(self):
        ...
        parser.add_argument(
            "--model_file", default="maskrcnn_benchmark/detectron_weights/detectron_model.pth", type=str, help="Detectron model file"
        )
        parser.add_argument(
            "--config_file", default="maskrcnn_benchmark/detectron_weights/detectron_model.yaml", type=str, help="Detectron config file"
        )
    ...
    def extract_features(self):
        ...
        if os.path.isfile(image_dir):
            ...
            for path, directories, files in os.walk(image_dir):
                new_files = glob.glob(os.path.join(path, "*.*"))
                print(new_files)
                files_1 = files_1 + new_files
            #files = glob.glob(os.path.join(image_dir, "*.*"))
            #print(files_1)
            for chunk in self._chunks(files_1, self.args.batch_size):
                features, infos = self.get_detectron_features(chunk)
                for idx, file_name in enumerate(chunk):
                    self._save_feature(file_name, features[idx], infos[idx])


def do_frcnn(input_dir="data/images", output_dir='data/frcnn'):
